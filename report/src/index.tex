...
The user layer encompases the user-interface and all mechanical systems that the user
interacts with such as the physical cash register, or touch screen. This portion of the
system is almost purly ornamental, but serves the important role of being the way in which
the system is experienced. Our team is using Svelte script to create the user-interface, which
is the only part of the user layer which is part of the app. Svelte can create complex
user interfaces reletively quick and easily, which makes it ideal for usage.\par

The application layer is the portion of the app that does the "thinking" so to speak.
While exact data structures and implementations have not been finalized, the theory
is to have a list of items which can be edited within the frontend, until that list
is finalized and sent over the network to the database as a list of required updates to make.
This process happens before payment in attempt to prevent miscounts.\par

The data layer is contained almost entierly within the backend, it stores inventory
along with UPC's (Universal Product Code(s)) and price for each item. This portion of the
software will be done with MongoDB through rust.
...
While the exact data structures are not fully in place, the idea to use MongoDB
to create a NoSQL path to mass data storage, that can be fed lists of items
and edited without the potential for missed data.
